...
export class Posts {
    ...
    switch (type) {
      case 'posts':
        ...
        if (check_posts.has(page)) {
          ...
          let description: string = post_info[<any>'description'];
      ...
      case 'guides':
        ...
        if (check_guides.has(page)) {
          ...
          let description: string = guides_info[<any>'description'];
          let blog: string = JSON.parse(feed)['title'];
  ...
  Write(content: any) {
    ...
    if (content['unpublish']) {
      delete fetch_feed[content['type']][content['unpublish']];
  ...
  Process(fetched_content: string) {
  ...
  async Fetch(code: string) {
    try {
      return await fetch(`${<string>process.env.PUBLISHED}${code}`);
    } catch (err) {
      return 'Failed';
    }

  }
  async Auth(info: string, provided_key: string) {
    ...
    let content_text: string = await content.text();

    this.Process(content_text);
